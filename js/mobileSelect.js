...
window.MobileSelect = (function() {
	...
	MobileSelect.prototype = {
		...
		checkCascade: function(){
			...
			if(_this.jsonType){ 
				...
				for(var i=0; i<node.length; i++){
					if('districtList' in node[i] && node[i].districtList.length > 0){
						_this.cascade = true;
						break;
					}
				}
		...
		checkArrDeep: function (parent) { //检测子节点深度  修改 displayJson
			var _this = this;
			if ('districtList' in parent && parent.districtList.length > 0) {
				_this.displayJson.push(_this.generateArrData(parent.districtList)) ; //生成子节点数组
				_this.checkArrDeep(parent.districtList[0]);//检测下一个子节点
			}
		},
		...
		checkRange: function(index, posIndexArr){
			...
			for (var i = 0; i <= index; i++){
				if (i == 0)
					...
					resultNode = resultNode.districtList[posIndexArr[i]];
				}
			}
