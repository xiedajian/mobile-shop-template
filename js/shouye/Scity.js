$(function(){
	var citys = [
		...
		var arrA=['A','B','C','D','E','F','G','H','J','K','L','M','N','P','Q','R','S','T','W','X','Y','Z'];
		var city_arr=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]];
		...
		for(var i in arrA){
			...
			var now1=i;
			for(var j in citys){
					if(citys[j].pinyin.charAt(0)==temp){
						city_arr[now1].push(citys[j].name);						
					}
			}
		}
		console.log(city_arr);		
		for(var i in city_arr){
			var len=city_arr[i].length;
			var now2=i;
			len>85?len=85:len=len;
			for(var j=0;j<len+1;j++){
				$('.city_name').eq(now2).append($('<li></li>'));
			}
			$('.city_name').eq(now2).children().eq(0).attr("id",arrA[now2]+'1').text(arrA[now2]);
			var city_name_leg=city_arr.length;
				for(var i=0;i<city_name_leg;i++){
					var ddd=$('.city_name').eq(i).children();
					var ttt=city_arr[i];		
					for(var j=0;j<len;j++){
						ddd.eq(j+1).text(ttt[j]);
					}
				};
		}
//		var city_name_leg=city_arr.length;
...
//		};
	...
	//
      ...
      //遍历每个存下来的城市 显示在历史记录中。
	for (var hh=0;hh<=$("#main_city li").length;hh++){
        var lishicity=localStorage.getItem('city_name'+hh+'');
       if(lishicity!=null){
            var lishi=$('<li class="mycity_li shengcheng"><div>'+lishicity+'</div></li>');
    ...
    //找到每个城市，可选择。，存在本地。
    $("#main_city").find('li').addClass('mycity_li')
    $(".mycity_li").click(function(){
        if($(this).attr('id')==undefined){
            var cityName_text=$(this).text();
          var lishi_i=$("#main_city li").index(this)
            localStorage.setItem('city_name',cityName_text);
            window.location.href='shouye.html';
            localStorage.setItem('city_name'+lishi_i,cityName_text);
        }
    });
	//遍历历史城市，将。当前的城市样式给加上。
    var lishiactive=localStorage.getItem('city_name')
    var lishibox=document.getElementById('lishijilu').getElementsByTagName('li');
    if (lishiactive!=null) {
        for (var ll = 0; ll < lishibox.length; ll++) {
            if (lishibox[ll].firstChild.innerHTML == lishiactive) {
                $(lishibox[ll]).addClass('selected');
            }
           // 历史记录去重
            var num = lishibox[ll].firstChild.innerHTML;
            var c = 0;
            for (var lll = 0; lll < lishibox.length; lll++) {
                if (num == lishibox[lll].firstChild.innerHTML) {
                    c++;
                    if (c > 1) { //重复时切掉
                       (lishibox[lll]).remove();
                    }
         ...
         navBar.on("touchmove", function (e) {
             ...
             $(this).find("a").each(function (i, item) {
                 ...
                 if (x > left && x < (left + width) && y > top && y < (top + height)) {
                     location.href = item.href;//页面跳转
                     $('.prompt').html($(item).html());
                 }
             });
         });
   ...
   //将定位的城市写在定位中。
    var gpsdingwei=localStorage.getItem('city_namegps')
	console.log(gpsdingwei)
    if (gpsdingwei!=null){
        $('#gpscity').html(gpsdingwei);
    }
